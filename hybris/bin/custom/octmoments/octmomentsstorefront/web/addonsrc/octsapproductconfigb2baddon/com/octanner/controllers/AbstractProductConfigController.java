...
import com.octanner.util.impl.PathExtractor;
...
public abstract class AbstractProductConfigController extends AbstractPageController
{
			...
			final Model model, final UiStatus uiStatus, final String cartEntryHandleFromRequest)
	{
		...
		model.addAttribute(BindingResult.MODEL_KEY_PREFIX + Octsapproductconfigb2baddonConstants.CONFIG_ATTRIBUTE, errors);

	}

	private BindingResult getBindingResultForConfig(final ConfigurationData configData)
	{
		final BindingResult errors = new BeanPropertyBindingResult(configData, Octsapproductconfigb2baddonConstants.CONFIG_ATTRIBUTE);

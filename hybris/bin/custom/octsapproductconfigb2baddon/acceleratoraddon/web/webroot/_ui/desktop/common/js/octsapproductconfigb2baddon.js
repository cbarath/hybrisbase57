var octsapproductconfigb2baddonConstants = {};
var scrollListenerSet = false;
var sideBarHeight = 0;

octsapproductconfigb2baddonConstants.init = function(baseUrl, addToCartText,
		updateCartText) {
	octsapproductconfigb2baddonConstants.baseUrl = baseUrl;
	octsapproductconfigb2baddonConstants.addToCartText = addToCartText;
	octsapproductconfigb2baddonConstants.updateCartText = updateCartText;
	octsapproductconfigb2baddonConstants.ajaxRunning = false;
	octsapproductconfigb2baddonConstants.ajaxRunCounter = 0;
	octsapproductconfigb2baddonConstants.resizeing = false;
};

function getConfigureUrl() {
	return octsapproductconfigb2baddonConstants.baseUrl + "/config";
}
function getConfigureRedirectUrl() {
	return octsapproductconfigb2baddonConstants.baseUrl + "/redirectconfig";
}
function getAddToCartUrl() {
	return octsapproductconfigb2baddonConstants.baseUrl + "/addToCart";
}
function getResetUrl() {
	return octsapproductconfigb2baddonConstants.baseUrl + "/reset";
}
function getCopyUrl() {
	return octsapproductconfigb2baddonConstants.baseUrl + "/copy";
}

function getAddToCartText() {
	return octsapproductconfigb2baddonConstants.addToCartText;
}
function getUpdateCartText() {
	return octsapproductconfigb2baddonConstants.updateCartText;
}
...
function firePost(fx, args) {
	octsapproductconfigb2baddonConstants.ajaxRunCounter++;
	waitToFirePost(fx, args);	
}

function waitToFirePost(fx, args) {
	if (octsapproductconfigb2baddonConstants.ajaxRunning == true) {
		setTimeout(function() {
			waitToFirePost.call(this, fx, args)
		}, 100);
	} else {
		octsapproductconfigb2baddonConstants.ajaxRunning = true
		// setTimeout(function() {
		fx.apply(this, args);
		// }, 50);
	}
...
function doUpdatePost(e, data, openErrorGroups, focusId) {
	$
			.post(
					...
					data,
					function(response) {
						if (octsapproductconfigb2baddonConstants.ajaxRunCounter == 1) {
							var focusElementId;
							var scrollTo;
							if (focusId) {
...
function initConfigPage() {
	$(window).resize(function() {
		...
		if (octsapproductconfigb2baddonConstants.resizeing) {
			return;
		}

		octsapproductconfigb2baddonConstants.resizeing = true;
		checkSidebarHeight();
	});
